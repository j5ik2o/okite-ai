---
name: avoiding-ambiguous-suffixes
description: >-
  命名・設計レビューにおいて、Manager / Util / Facade / Service / Runtime / Engine など
  意味が一意に定まらないサフィックスの使用を検出し、責務・境界・契約が明確に伝わる命名へ導く。
  トリガー：「この命名で良いか」「Manager という名前を使いたい」「命名レビューして」
  「サフィックスの選び方」といった命名相談で起動。
---

# 曖昧なサフィックスを避ける

命名・設計レビューにおいて曖昧なサフィックスを検出し、明確な命名へ導く。
