...
| DDD定義 | Eric Evans | ドメインモデリング | ドメインの概念を計測・定量化・説明し、不変条件と副作用のない振る舞いを持つドメインオブジェクト |

### 重要な違い

- **PofEAA版**: 等価性の意味論（値で比較する）に焦点。ドメイン不変条件は必須要件ではない
- **DDD版**: ドメインの概念表現に焦点。不変条件の維持と振る舞いの集約がセット

この違いを認識せずに議論すると噛み合わない。**チーム内では「PofEAAのVO」「DDDのVO」のように文脈を明示して使い分けるべき。**
...
| Value Object（DDD） | ドメインの概念を表現し、不変条件と振る舞いを持つ値 |
