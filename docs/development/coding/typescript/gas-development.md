...
- GAS には本格的なイベントループが存在せず、他の処理が並列に実行されることはありません
- async/await を使用しても、実行は基本的に同期的にブロックされます
- このため、同期処理パターンを前提としたコードを使用することを推奨します

### 2.2 ES6+機能の制限

- 一部のモダンな JavaScript 機能が利用できない場合があります
- ES6 以降の機能はサポートされていますが、一部の機能（例：dynamic import）は利用できません
- トランスパイルが必要な場合があります
- モジュールシステムはサポートされていますが、従来の GAS とは異なる方法で実装されています
...
- トリガーを使用して長時間の処理を小さなタスクに分割することができます
...
- スタンドアロンスクリプト：特定のドキュメントに紐づかない独立したスクリプト
...
- TypeScript を使用する場合は、GAS の制約を考慮した tsconfig.json 設定を行う
...
- `npm run build`: TypeScriptの型チェックとesbuildによるバンドルを実行
...
Biomeはpackage.jsonに記載されている通り、`npm run lint`や`npm run lint:fix`コマンドで実行できます。ESlintよりも高速で、TypeScriptとの統合もスムーズなため、GAS開発のような中小規模プロジェクトに適しています。
